...
export const splBracketProcess = (
  ...
  if (splBracket.winner === DQ.ScoreNotUploaded) {
    if (nextEmptyBracketIndex !== -1) {
      const emptyBracketUserIndex = getUserIndex(
        users,
        brackets[nextEmptyBracketIndex].teamA.user
      );
      if (emptyBracketUserIndex !== -1) {
        brackets[nextEmptyBracketIndex].winner =
          users[emptyBracketUserIndex]?.ughId;
        const nextPlayEmptyBracketIndex = getNextEmptyBracketIndex(
          brackets,
        ...
        if (nextPlayEmptyBracketIndex !== -1) {
          ...
          bracketCheckTimer(
            brackets[nextPlayEmptyBracketIndex].regId,
            bracketCheckTime,
            tournament.regId
          );
  ...
  //      - start check timer
    ...
    if (splBracketWinnerIndex !== -1) {
      ...
      bracketCheckTimer(
        brackets[nextEmptyBracketIndex].regId,
        bracketCheckTime,
        tournament.regId
      );
    }
  ...
  //  - process all brackets to declare winner
    ...
    if (winner1UserIndex !== -1) {
      ...
      users[winner1UserIndex].tournaments = users[
        winner1UserIndex
      ].tournaments.map((t) => {
    ...
    if (tournament.winnerCount === 2) {
      const winner2User =
        JSON.stringify(users[winner1UserIndex].id) ===
        JSON.stringify(splBracket.teamA.user)
          ? splBracket.teamB.user
          : splBracket.teamA.user;
      ...
      if (winner2UserIndex !== -1) {
        ...
        users[winner2UserIndex].tournaments = users[
          winner2UserIndex
        ].tournaments.map((t) => {
          if (JSON.stringify(t.id) === JSON.stringify(tournament.id)) {
