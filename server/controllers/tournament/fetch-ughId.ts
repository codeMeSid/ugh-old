...
export const tournamentFetchUghIdController = async (
  req: Request,
  res: Response
) => {
  ...
  //       _id: { $in: tournamentsCount.map((t) => mongoose.Types.ObjectId(t.id)) },
  //     })
  //       .select("name startDateTime game")
  //       .populate("game", "name", "Games");
  //     user.tournaments = user.tournaments.map((t) => {
  //       const tIndex = tournaments.findIndex(
  //         (p) => JSON.stringify(p.id) === JSON.stringify(t.id)
  //       );
  //       if (tIndex >= 0) {
  ...
  //     });
  //   }
  res.send(user.tournaments.reverse());
