...
export const tournamentLeaveHandler = async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (tournament.status !== TournamentStatus.Upcoming)
      throw new BadRequestError("Cannot leave tournament at this stage");
    if (
      new Date(tournament.startDateTime).valueOf() - Date.now() <
      1000 * 60 * 30
    )
      throw new BadRequestError(
        "Deadline to back out it over. Cannot escape now."
      );
    ...
    tournament.players = tournament.players.filter(
      (pId) => JSON.stringify(pId) !== JSON.stringify(user.id)
    );
    ...
    user.wallet.coins += tournament.isFree
      ? 0
      : tournament.coins - penaltyCoins;

    const passbook = Passbook.build({
      coins: tournament.isFree ? 0 : penaltyCoins,
