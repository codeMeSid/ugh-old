      ...
      <div className="match">
        <div className="match__head">
          <div className="match__head__container">
            <div className="match__head__container__title">our tournaments</div>
            ...
            <div className="match__head__container__list">
              <div className="match__head__container__list__item">
                <div
                  className={`match__head__container__list__item__image ${
                    game === "all" ? "active" : ""
                  } button`}
                >
                  <Button
              ...
              {Object.keys(tournaments).map((key) => {
                ...
                return (
                  <Tooltip key={key} title={gameName}>
                    <div
                      onClick={() => setGame(gameName)}
                      className="match__head__container__list__item"
                    >
                      <img
                        src={imageUrl}
                        alt={gameName}
                        className={`match__head__container__list__item__image ${
                          game === gameName ? "active" : ""
                        }`}
                      />
                    </div>
                  </Tooltip>
            ...
            <ul className="match__footer__title__side">
              {["upcoming", "started", "completed"].map((t) => {
                return (
                  <li
                    onClick={() => setType(t)}
                    className={`match__footer__title__side__item ${
                      type === t ? "active" : ""
                    }`}
                    key={t}
                  >
...
Tournaments.getInitialProps = async (ctx) => {
  const {
    data,
    errors,
  }: { data: Array<TournamentDoc>; errors: any } = await serverRequest(ctx, {
    url: "/api/ugh/tournament/fetch/all/my",
    body: {},
    method: "get",
  });
  const tournaments = {};
