...
const SponsorDetail = ({ sponsor, errors }) => {
  ...
  const socialAddHandler = () => {
    ...
    if (!urlRegex.test(link))
      return setMessages([{ message: "Invalid url format" }]);
  ...
  const { doRequest } = useRequest({
    url: `/api/ugh/sponsor/update/detail/${sponsor.sponsorId}`,
    body: {
      name,
      website,
      imageUrl,
  ...
  return (
    <MainLayout messages={messages}>
      <div style={{ margin: "5rem auto" }}>
        <div style={{ textAlign: "center", fontSize: 38 }}>New Sponsor</div>
          ...
          <div className="row">
            <div className="col">
              <Input
                ...
                value={sponsor?.sponsorPack?.pack?.duration}
                disabled
              />
            </div>
            <div className="col">
              ...
              {socialList.length > 0 && (
                <Table
                  data={socialList.map(({ name, href }, index) => [
                    ...
                    <FaTrash
                      onClick={() =>
                        setSocialList(socialList.filter((s, i) => i !== index))
                      }
                    />,
                  ])}
                  headers={[
                    { isResponsive: false, text: "social media" },
                    { isResponsive: false, text: "link" },
                    { isResponsive: false, text: "remove" },
                  ]}
                  hasLoader
                />
              )}
          ...
          <div className="row">
            <div className="col">
              <Select
                ...
                onSelect={(A) => setSocial(A.currentTarget.value)}
                options={[
                  "facebook",
                  "twitter",
                  "discord",
              ...
              onPress={async (_, next) => {
                if (!sponsor)
                  return setMessages([
                    {
                      message:
                        "Invalid Link! Kindly ask admin to generate new link",
                    },
                  ]);
...
SponsorDetail.getInitialProps = async (ctx) => {
  ...
  const { data, errors } = await serverRequest(ctx, {
    url: `/api/ugh/sponsor/fetch/${sponsorId}`,
    body: {},
    method: "get",
  });
