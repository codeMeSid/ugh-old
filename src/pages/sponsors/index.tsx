...
const Sponsors = ({
  ...
  sponsors: any;
  sponsorships: Array<SponsorshipDoc>;
}) => {
        ...
        <div className="sponsors__head">
          ...
          <div className="sponsors__new">
                ...
                return sponsorship?.packs?.map((pack, pIndex) => {
                  return (
                    <Option
                      ...
                      key={Math.random()}
                      display={`${sponsorship?.name?.toUpperCase()} (${
                        pack?.duration
                      } months - ₹${pack?.price})`}
                      value={`${sIndex},${pIndex}`}
                    />
            ...
            <textarea
              ...
              name="message"
              onChange={(e) =>
                onChangeHandler(e.currentTarget.name, e.currentTarget.value)
              }
            />
            <div
              style={{
            ...
            >
                ...
                onPress={async (_, next) => {
                  const { doRequest } = useRequest({
                    ...
                    body: {
                      ...
                      phone,
                      duration:
                        sponsorships[sponsorshipIndex]?.packs[packIndex]
                          ?.duration,
                      price:
                        sponsorships[sponsorshipIndex]?.packs[packIndex]?.price,
                    ...
                    onSuccess: () => {
                      setMessages([
                        { message: "Request submitted", type: "success" },
                      ]);
              ...
              >
                <div style={{ padding: "0rem 1rem" }}>
                  <RichText
                    content={sponsorships[sponsorshipIndex]?.description}
                  />
                </div>
              </DialogButton>
            </div>
        ...
        <div className="sponsors__body">
          <div className="sponsors__body__container">
            {Object.keys(sponsors || {})?.map((sponsorKey) => {
              return (
                <div key={sponsorKey} className="sponsors__group">
                  <div className="sponsors__group__title">
                    {sponsorKey} members
                  </div>
                  <div className="sponsors__group__list">
                    {Array.from(sponsors[sponsorKey]).map(
                      (sponsor: SponsorDoc) => {
                        return (
                          <SponsorCard key={Math.random()} sponsor={sponsor} />
                        );
                      }
                    )}
                  </div>
...
Sponsors.getInitialProps = async (ctx) => {
  const sponsors = await serverRequest(ctx, {
    url: "/api/ugh/sponsor/fetch/active",
    body: {},
    method: "get",
  });
  const sponsorships = await serverRequest(ctx, {
    url: "/api/ugh/sponsorship/fetch/active",
    body: {},
    method: "get",
  });
  const packSponsors = {};
  Array.from(sponsors?.data || []).forEach((sponsor: SponsorDoc) => {
    if (packSponsors[sponsor.sponsorPack.name]) {
      packSponsors[sponsor.sponsorPack.name] = [
        ...packSponsors[sponsor.sponsorPack.name],
        sponsor,
      ];
    } else {
