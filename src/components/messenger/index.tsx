...
class MessengerList extends Component<Props> {
  ...
  componentDidMount() {
    event.recieveMessage((data) => {
      ...
      this.setState({
        messengers: uMessengers.sort((a, b) => b.count - a.count),
      });
    });
  }
  ...
  componentDidUpdate(prevProps: Props) {
    ...
    if (JSON.stringify(chats) !== JSON.stringify(this.props.chats))
      this.setState({
        messengers: this.formatChat(this.props.chats),
  ...
  render() {
    ...
    const mNotificationCount = messengers.reduce(
      (acc, mess) => acc + mess.count,
      0
    );
    const from = () => {
      ...
      if (!chat) return from;
        ...
        chat.channel === SocketChannel.Match &&
    ...
    return !isOpen ? (
      <>
        <div
          className="messenger__window"
          onClick={() => this.setState({ isOpen: true })}
        >
          <SiGooglemessages className="messenger__window__icon" />
          {mNotificationCount > 0 && (
            <div className="messenger__window__count">{mNotificationCount}</div>
          )}
        </div>
      </>
    ) : (
      <>
        <div className={`messenger__contact ${isContactOpen ? "active" : ""}`}>
          <div className="messenger__contact__head">
            <GiHamburgerMenu
              className="messenger__contact__head__icon"
              onClick={() =>
                this.setState((ps: any) => ({
                  isContactOpen: !ps.isContactOpen,
                }))
              }
            />
          ...
          <div className="messenger__contact__list">
            {messengers.map((chat, index) => {
              return (
                ...
                >
                  <div
                    className={`messenger__contact__item__head ${
                      chat?.profile ? "" : "none"
                    }`}
                    style={{
                      backgroundImage: `url(${chat?.profile || UserProfile})`,
                    }}
                  >
                    {chat.count > 0 && (
                      <div className="messenger__contact__item__head__count">
                        {chat.count}
                      </div>
                    )}
                  </div>
                  <div className="messenger__contact__item__title">
                    {chat.title}
                  </div>
                </div>
              );
        ...
        <MessengerChat
          ...
          onOpenMenu={() =>
            this.setState((ps: any) => ({ isContactOpen: !ps.isContactOpen }))
          }
          from={from()}
          to={chat?.to}
