...
const TournamentCard = ({ match }: { match: TournamentDoc }) => {
  const getTimer = () => {
    ...
    if (match.status === "completed") return <div>Completed</div>;
      ...
      return (
        <Timer
          canCountdown={!!match.startDateTime}
      ...
      return (
        <Timer
          canCountdown={!!match.endDateTime}
          dateTime={match.endDateTime}
        />
      );
  };

  return (
    <Link href={`/tournaments/${match.regId}`}>
      <a className="match__footer__list__item">
        ...
        <img
          src={match.game.imageUrl}
          alt={match.game.name}
          className="match__footer__list__item__image"
        />
        ...
        <div className="match__footer__list__item__entry">
          ...
          {match?.isFree ? <div>Free</div> : <div>{match.coins} coins</div>}
        </div>
      </a>
    </Link>
  );
